...
class StokOpname extends CI_Controller{
  ...
  function index()
  {
    $data = array(
      ...
      'opname' => $this->ModelStokOpname->get_list()
    );
  ...
  function list_batch()
  {
    ...
    if (empty($data)) {
      $data = $this->ModelStokOpname->list_batch_baru($idobat)->result();
    }
    // die(var_dump($data));
    $html = "";
    foreach ($data as $value) {
      $html .= "<tr>
      <td><input hidden value='".$value->iddetail_pembelian_obat."' name='id_pengadaan[]'><input hidden value='".$value->idobat."' name='kode_obat[]'><input type='hidden' name='nama[]' value='".$value->nama_obat."'>".$value->nama_obat."</td>
      <td><input type='hidden' name='no_batch[]' class='form-control' value='".$value->no_batch."'>".$value->no_batch."</td>
      <td><input type='hidden' id='harga_beli".$value->iddetail_pembelian_obat."' name='harga_beli[]' class='form-control' value='".$value->harga_beli."'>".$value->harga_beli."</td>
      <td><input type='hidden' name='satuan[]' class='form-control' value='".$value->satuan."'>".$value->satuan."</td>
      <td><input type='number' id='jumlah_komp".$value->iddetail_pembelian_obat."' class='form-control jml_beli' name='jumlah_komp[]' readonly value='".$value->stok."'></td>
      <td><input type='number'min='0' id='jumlah_real".$value->iddetail_pembelian_obat."' class='form-control jml_beli' name='jumlah_real[]' onkeyup='hitung(`".$value->iddetail_pembelian_obat."`)'></td>
      <td><input type='number' readonly id='selisih".$value->iddetail_pembelian_obat."' class='form-control jml_beli' name='selisih[]'></td>
      <td ><input type='hidden' id='selisih_harga".$value->iddetail_pembelian_obat."' class='form-control jml_beli' name='selisih_harga[]'><span id='tx_selisih_harga".$value->iddetail_pembelian_obat."''></span></td>
  ...
  function insert()
  {
    ...
    for ($i=0; $i < count($id); $i++) {
      $data_pembelian = $this->db->where("iddetail_pembelian_obat",$id[$i])->get("detail_pembelian_obat")->row();
      $data = array(
  ...
  function cetak(){
    ...
    $excel = new PHPExcel();
    $data = $this->ModelStokOpname->get_opname($tanggal);
    // die(var_dump($data));
    //Merge cells
    ...
    foreach ($data as $value)
    {
          ...
          $excel->getActiveSheet()->setCellValueByColumnAndRow(2, $i,$value->obat_idobat);
  ...
  public function set_opname(){
    ...
    ->get_where("stok_opname",array("stok_opname"=>NULL,"stok_opname.tanggal"=>"2020-08-30"))->result();
    // var_dump($data);
    // die();
    foreach ($data as $value) {
      // $pengadaan = $this->db->get_where("detail_pembelian_obat",array("iddetail_pembelian_obat"=>$value->id_pengadaan))->row();
      // $stok_opname = $value->selisih;
      // var_dump($stok_opname);
      // die();
      // echo $value->id_pengadaan." | ".$stok_opname."<br>";
      // $this->db
      // ->where("iddetail_pembelian_obat",$value->id_pengadaan)
      // ->update("detail_pembelian_obat",array("stok_opname"=>$stok_opname));
