<div class="main-content">
  <div class="section__content section__content--p30">
    <div class="container-fluid">
      <?php echo form_open_multipart('Periksa/input_ditin');?>
      <div class="row">
          ...
          <div class="card">
            ...
            <div class="card-footer" >
              <div class="col col-sm-12 com-md-12">
                <button type="button" class="btn btn-outline-secondary btn-sm pull-left waves-effect waves-light" onclick="edit_periksa('<?php echo $data_periksa['kunjungan_no_urutkunjungan']; ?>')"><i class="fa fa-reply"></i> Edit Periksa</button>
                <button type="submit" class="btn btn-primary btn-sm pull-right waves-effect waves-light">SIMPAN</button>
              </div>
            </div>
          </div>
        </div>
...
<div class="modal fade" id="modaljapel" tabindex="-1" role="dialog" aria-labelledby="modaljapelLabel" aria-hidden="true">
              ...
              <?php $no=1; foreach ($modaljapel as $data) {?>
                ...
                <tr>
                  ...
                  <td><button type="button"
                    onclick="select_jasa('<?php echo $data->kode_jasa?>', '<?php echo $data->nama?>', '<?php echo $data->hrg_1; ?>', '<?php echo $data->jasa_dokter_1; ?>', '<?php echo $data->jasa_perawat_1; ?>','<?php echo $data->jasa_admin_1; ?>', '<?php echo $data->jasa_resepsionis_1; ?>', '<?php echo $data->kode_bpjs; ?>' )"
  ...
  <div class="modal fade" id="modal_edit_periksa" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-fluid modal-notify modal-info" role="document">
      <!--Content-->
      <div class="modal-content hasil">
...
<script type="text/javascript">
  ...
  function edit_periksa(kunjungan_no_urutkunjungan) {
    $.ajax({
      type : 'POST',
      url : '<?php echo base_url() ?>Periksa/data_modal',
      data : {kunjungan_no_urutkunjungan:kunjungan_no_urutkunjungan},
      success:function(response){
        // console.log(response);
        $('.hasil').html(response);
        $('#modal_edit_periksa').modal('show');
      }
    });
  }
  ...
  function select_diagnosa(kode,sakit) {
    ...
    var markup = "<tr>" +
      ...
      function select_jasa(kode, jasa, harga, japeldokter, japelperawat, japeladmin, japelresepsionis,kode_bpjs) {
        var kode;
        var jasa;
        var harga;
        var japeldokter;
        ...
        var markup = "<tr>" +
          "<td class='text-right'><input hidden value='"+ kode_bpjs +"' name='kode_bpjs[]'><input hidden value='"+ kode +"' name='kode_jasa[]'>"+ kode +"</td>" +
