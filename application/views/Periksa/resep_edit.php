<div class="main-content">
  <div class="section__content section__content--p30">
    <div class="container-fluid">
      <?php echo form_open_multipart('Periksa/update_resep/'.$this->uri->segment(4),array("id"=>"form_resep"));?>
      <div class="row">
        <div class="col-lg-12">
                  ...
                  <tbody id="alergi">
                    <?php if ($this->uri->segment(4)!=''): ?>
                      <?php foreach ($riwayat as $value): ?>
                        <tr>
                          <td><?php echo $value->alergi_reaksi ?></td>
                          <td><?php echo $value->konsumsi ?></td>
                          <td><?php echo $value->jenis_penyakit ?></td>
                          <td><?php echo $value->tgl_alergi ?></td>
                        </tr>
                      <?php endforeach; ?>
                    <?php endif; ?>
                  </tbody>
                    ...
                    <tbody id="resep">
                      <?php foreach ($resep_lama as $value):
                        ...
                        $pr = '';
                         ...
                         if ($_SESSION['poli']=="UGD") {
                           $stok = $value->stok_berjalan;
                         }else{
                           $stok = $value->stok_berjalan_ugd;
                         }
                        ?>
                        <tr>
                          ...
                          <td><input hidden value='<?php echo $stok ?>' name='stok_tersedia[]' id='stok_<?php echo $pr;?>'><?php echo $stok?></td>
                          <td><input hidden value='<?php echo $value->harga?>' name='harga[]' id='hrg_<?php echo $pr.$id;?>'>Rp.<?php echo number_format($value->harga)?></td>
                          <td><input type='number' min="1" obat="<?php echo $id?>" value='<?php echo $value->jumlah?>' name='jumlah[]' id='<?php echo $pr.$id;?>' class='input_jumlah form-control' required  style='max-width:100px;'></td>
    ...
    <input type="hidden" id="idresep" value="<?php echo $this->uri->segment(4)?>">
    <?php echo form_close(); ?>
  </div>
</div>
...
<div class="modal fade" id="scrollmodal" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabel" aria-hidden="true">
  ...
  function tambahobat(res) {
    ...
    "<td><input type='number' value='' min='1' obat='"+kode+"' name='jumlah[]' id='"+pr+kode+"' class='input_jumlah form-control' required  style='max-width:100px;'></td>" +
  ...
  $(document).on("click",".simpan",function(){
    ...
    $(".input_jumlah").each(function(){
      ...
      var th = $(this);
      if (jumlah=='' || jumlah<0) {
        alert("Jumlah Resep Tidak Boleh Minus")
        th.focus();
        status = 1;
        return false;
      }
      ...
      $.ajax({
        type: 'POST',
        url: '<?php echo base_url();?>Obat/cek_stok_edit/' + kode,
        data: { idobat: kode,jumlah:jumlah,no_resep:idresep},
        dataType:'json',
        async : false,
        success: function(response) {
          // alert(response.kode);
